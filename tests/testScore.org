...
a <- Score(list(f1,f2,f3),data=test.d,formula=Y~1,metrics=c("Brier","auc"),conf.int=FALSE,nullModel=FALSE)
library(ModelGood)
...
f2 <- lrm(Y~X1+X2+X9,data=train.d)
f3 <- lrm(Y~X6,data=train.d)
Source(riskRegression);testr <- Score(list(f1,f2,f3),data=test.d,formula=Y~1,metrics=c("Brier","auc"),test=FALSE,nullModel=FALSE)
...
print(rbind(c(Package="timeROC",timeROC.AUC=tROC$AUC[[2]],
            ...
            c(Package="riskRegression",Rscore$AUC$score[,c("AUC"=AUC,"se.AUC"=se.AUC)])))
...
## [3,]  0.8 -0.2 -0.2 -0.2 -0.2
## [4,]  0.8 -0.2 -0.2 -0.2 -0.2
a1 <- Score(list(mini$X1),data=mini,formula=Surv(time,status)~1,times=c(3.4),metrics="AUC",compare=FALSE,nullModel=FALSE)
...
mini <- data.frame(time=c(4,2,3,3,7),status=c(1,0,1,1,1),X2=c(-.1,0.1,-0.03,0.04,0.3),X1=c(7,1,9,4,2))
mini <- mini[order(mini$time),]
a <- Score(list(mini$X1,mini$X2),data=mini,formula=Surv(time,status)~1,times=c(3.4),metrics="AUC",compare=FALSE,nullModel=FALSE)
a1 <- Score(list(mini$X1),data=mini,formula=Surv(time,status)~1,times=c(3.4),metrics="AUC",compare=FALSE,nullModel=FALSE)
b1 <- with(mini,timeROC(T=time,delta=status,marker=X1,cause=1,times=3.4,iid=TRUE))
...
cbind(seAUC.Score=a$AUC$score[,se.AUC],seAUC.timeROC=c(b1$inference$vect_sd_1[[2]],b2$inference$vect_sd_1[[2]]))
#+END_SRC
...
print(cbind(TROC=c(BS=a$BS,se=a$sd),Score=b$Brier$score[,c(Brier,se.Brier)]))
#+END_SRC
